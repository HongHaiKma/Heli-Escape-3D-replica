...
public class RopeControllerSimple : MonoBehaviour 
{
    ...
    void Start() 
	{
        springJoint = tf_Origin.GetComponent<SpringJoint>();
        ...
        tf_Target.GetComponent<Rigidbody>().mass = loadMass;
    }
    ...
    private void DisplayRope()
    {
        ...
        Vector3 A = tf_Origin.position;
        Vector3 D = tf_Target.position;

        //Upper control point
        //To get a little curve at the top than at the bottom
        Vector3 B = A + tf_Origin.up * (-(A - D).magnitude * 0.1f);
        //B = A;

        //Lower control point
        Vector3 C = D + tf_Target.up * ((A - D).magnitude * 0.5f);
        ...
        //positions[0] = tf_Origin.position;
        //positions[1] = tf_Target.position;
